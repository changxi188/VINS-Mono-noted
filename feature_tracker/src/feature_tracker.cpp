...
void FeatureTracker::readImage(const cv::Mat& _img, double _cur_time)
{
    ...
    if (EQUALIZE)
    {
        ...
        LOG(INFO) << "readImage --- CLAHE costs: " << t_c.toc() << "ms";
    }
    else
    {
        img = _img;
    ...
    rejectWithF();
    LOG(INFO) << "readImage --- set mask begins";
    TicToc t_m;
    setMask();
    LOG(INFO) << "readImage --- set mask costs " << t_m.toc() << "ms";

    LOG(INFO) << "readImage --- detect feature begins";
    TicToc t_t;
    ...
    LOG(INFO) << "readImage --- detect feature costs: " << t_t.toc() << "ms";

    LOG(INFO) << "readImage --- add feature begins";
    TicToc t_a;
    addPoints();
    LOG(INFO) << "readImage --- selectFeature costs: " << t_a.toc() << "ms";
...
void FeatureTracker::rejectWithF()
{
    ...
    if (forw_pts.size() >= 8)
    {
        LOG(INFO) << "rejectWithF --- FM ransac begins";
        TicToc              t_f;
...
{
    LOG(INFO) << "readIntrinsicParameter --- reading paramerter of camera " << calib_file.c_str();
    // 读到的相机内参赋给m_camera
...
void FeatureTracker::showUndistortion(const string& name)
{
    ...
    for (int i = 0; i < int(undistortedp.size()); i++)
    {
        ...
        {
            // LOG(ERROR) << "(%f %f) -> (%f %f)", distortedp[i].y, distortedp[i].x, pp.at<float>(1, 0), pp.at<float>(0,
            // 0));
        }
    }
