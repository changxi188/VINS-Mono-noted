...
void img_callback(const sensor_msgs::ImageConstPtr& img_msg)
{
    ...
    {
        // 一些常规的reset操作
        LOG(WARNING) << "img_callback --- image discontinue! reset the feature tracker!";
    ...
    {
        LOG(INFO) << "img_callback --- processing camera " << i;
        if (i != 1 || !STEREO_TRACK)
        {
    ...
    if (!PUB_THIS_FRAME)
    {
        LOG(WARNING) << "img_callback --- don't pub this frame, whole feature tracker processing costs: " << t_r.toc();
        return;
    }
    ...
    LOG(INFO) << "img_callback --- publish " << feature_points->header.stamp.toSec() << ", at "
              << ros::Time::now().toSec();
    ...
    LOG(INFO) << "img_callback --- whole feature tracker processing costs: " << t_r.toc();
}
...
int main(int argc, char** argv)
{
    ...
    if (FISHEYE)
    {
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            {
                LOG(INFO) << "main --- load mask fail";
                ROS_BREAK();
            }
            else
                LOG(INFO) << "main --- load mask success";
        }
    }
