...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo* residual_block_info)
{
    ...
    std::vector<int> parameter_block_sizes =
        residual_block_info->cost_function->parameter_block_sizes();  // 各个参数块的大小
...
void MarginalizationInfo::marginalize()
{
    ...
    for (auto& it : parameter_block_idx)
    {
        ...
        pos +=
            localSize(parameter_block_size[it.first]);  // 因为要进行求导，因此大小时local size，具体一点就是使用李代数
    }
    ...
    */
    ...
    // 所以J = S^(1/2) * VT , 这样JT * J = (S^(1/2) * VT)T * S^(1/2) * VT = V * S^(1/2)T *  S^(1/2) * VT = V * S *
    // VT(对角矩阵的转置等于其本身) e = -(JT)-1 * b = - (S^-(1/2) * V^-1) * b = - (S^-(1/2) * VT) * b
