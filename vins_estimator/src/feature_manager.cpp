...
bool FeatureManager::addFeatureCheckParallax(int frame_count,
                                             ...
                                             double                                                          td)
{
    LOG(INFO) << "addFeatureCheckParallax --- input feature: " << (int)image.size();
    LOG(INFO) << "addFeatureCheckParallax --- num of feature: " << getFeatureCount();
    ...
    for (const auto& it_per_id : feature)
    {
        ...
        // 因此起始帧至少得是frame_count - 2,同时至少连续匹配到frame_count - 1帧
    ...
    if (parallax_num == 0)
    {
        LOG(WARNING) << "addFeatureCheckParallax --- no feature track betwwen second last frame and third last frame";
        return true;
    }
    else
    {
        LOG(INFO) << "addFeatureCheckParallax --- parallax_sum: " << parallax_sum << ", parallax_num: " << parallax_num;
        LOG(INFO) << "addFeatureCheckParallax --- current parallax: " << parallax_sum / parallax_num * FOCAL_LENGTH;
        // 看看平均视差是否超过一个阈值
 ...
 * @brief
 *
 * @param[in] marg_R  被移除的位姿       R_wc0_old
 * @param[in] marg_P                     P_wc0_old
 * @param[in] new_R   转接地图点的位姿   R_wc0_new
 * @param[in] new_P                      P_wc0_new
 */
...
double FeatureManager::compensatedParallax2(const FeaturePerId& it_per_id, int frame_count)
{
    ...
    // 找到相邻两帧
    int                    third_last_frame_id  = frame_count - it_per_id.start_frame - 2;
    int                    second_last_frame_id = frame_count - it_per_id.start_frame - 1;
    const FeaturePerFrame& frame_i              = it_per_id.feature_per_frame[third_last_frame_id];
    const FeaturePerFrame& frame_j              = it_per_id.feature_per_frame[second_last_frame_id];
