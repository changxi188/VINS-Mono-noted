...
void new_sequence()
{
    LOG(INFO) << "new_sequence --- new sequence";
    sequence++;
    LOG(WARNING) << "new_sequence -- sequence cnt " << sequence;
    if (sequence > 5)
    {
        LOG(WARNING) << "new_sequence -- only support 5 sequences since it's boring to copy code for more sequences.";
        ROS_BREAK();
    }
...
void image_callback(const sensor_msgs::ImageConstPtr& image_msg)
{
    ...
    {
        LOG(WARNING) << "image_callback --- image discontinue! detect a new sequence!";
...
void process()
{
    ...
    while (true)
    {
        ...
        if (!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())
        {
            ...
            {
                pose_buf.pop();
                LOG(WARNING) << "process --- throw pose at beginning";
            }
            else if (image_buf.front()->header.stamp.toSec() > point_buf.front()->header.stamp.toSec())
            {
                point_buf.pop();
                LOG(WARNING) << "process --- throw point at beginning";
            }
...
void command()
{
    ...
    while (1)
    {
        ...
        if (c == 's')  // s就是存储地图
        {
            ...
            m_process.unlock();
            LOG(INFO) << "command --- save pose graph finish\nyou can set 'load_previous_pose_graph' to 1 in the "
                         "config file to reuse it "
                         "next time";
...
int main(int argc, char** argv)
{
    ...
    n.getParam("log_to_screen", log_to_screen);
    google::InitGoogleLogging(argv[0]);
    google::SetLogDestination(google::INFO, "./log/pose_graph/pose_graph.log");
    ...
    {
        LOG(FATAL) << "main --- Wrong path to settings" << std::endl;
    }
    // 可视化的参数
    ...
    if (LOOP_CLOSURE)
    {
        ...
        LOG(INFO) << "main --- vocabulary_file" << vocabulary_file;
        ...
        LOG(INFO) << "main --- BRIEF_PATTERN_FILE" << BRIEF_PATTERN_FILE;
        // 和前面一样，生成一个相机模型
        ...
        {
            LOG(INFO) << "main --- load pose graph";
            m_process.lock();
            posegraph.loadPoseGraph();
            m_process.unlock();
            LOG(INFO) << "main --- load pose graph finish";
            load_flag = 1;
        }
        else
        {
            LOG(INFO) << "main --- no previous pose graph";
            load_flag = 1;
        }
    }
